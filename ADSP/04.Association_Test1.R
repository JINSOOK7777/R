...
#subset은 조건문
(G_arules_yogurt <- subset(G_arules, subset = rhs %in% "yogurt" & lift > 2)) #()로 묶으면 변수 저장 뿐 아니라 결과 출력도 됨
...
# (h) 모델 시각화: 그룹 옵션으로 시각화    --- 가장 가독성 높음~
plot(G_arulesViz_sub1, method="grouped")

# (i) 모델 시각화: lift 상위 10개에 대해 그래프 옵션으로 시각화 --- 가장 가독성 높음~
